...
export class SwiftClient {
			...
			fetch(postRequest)
				.then(async (response) => {
					...
					try {
						...
						debugLog('swiftClient', 'Error reading response body', err);
					}
					res({
						success: response.ok,
						body: resBody,
	...
	static async sendSwiftOrder(
		...
		if (response.status !== 200) {
			...
			debugLog('swiftClient', 'full non-200 response body', response.body);
			return {
				message:
	...
	static findOrderInSignedMsgUserOrdersAccount(
		...
		debugLog('swiftClient findOrder', 'order\n', order);
		return order;
	}
	...
	static async handleSwiftOrderSubscriberWS(
		...
		debugLog('swiftClient', 'sendResponse\n', sendResponse);
		...
		const orderID = await this.confirmSwiftOrderWS(
			...
			debugLog('swiftClient', 'confirmSwiftOrderWS error', err);
			subscriber.next({
				type: 'expired',
				hash,
