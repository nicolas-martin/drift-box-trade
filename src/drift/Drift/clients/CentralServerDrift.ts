...
import { createPlaceholderIWallet } from '../../utils/createPlaceholderWallet';
import {
...
export class CentralServerDrift {
	 ...
	 * @param rpcApiKey - Optional API key for authenticated RPC endpoints (e.g., Chainstack).
	 */
	constructor(config: {
		...
		if (config.rpcApiKey) {
			connectionConfig.httpHeaders = {
				'Authorization': `Bearer ${config.rpcApiKey}`
			};
		}

		const connection = new Connection(config.solanaRpcEndpoint, connectionConfig);
