 ...
 *   RPC_API_KEY=your-api-key (optional, for authenticated endpoints like Chainstack)
 *
 * Usage:
...
const userAccountPublicKey = new PublicKey('DDq81uBv4LcgZmjBcRS2V5PMuNqc8tcsAzJBh4TCLp1g'); // Drift user account (subaccount 0)
...
async function initializeCentralServerDrift(): Promise<void> {
	...
	centralServerDrift = new CentralServerDrift({
		...
		rpcApiKey: process.env.RPC_API_KEY, // Optional API key for authenticated endpoints
...
async function executeVersionedTransaction(
	...
	console.log(
		`  First Signature Present: ${txn.signatures[0] && txn.signatures[0].length > 0
		}`
	);
	...
	console.log(`📋 Transaction Signature: ${txSig.txSig || txSig}`);
	console.log(`🔍 View on Solscan: https://solscan.io/tx/${txSig.txSig || txSig}`);
}
...
async function depositWithdrawExample() {
	...
	// WARNING: These are REAL FUNDS on mainnet!
	const amount = new BN(10000); // 0.01 USDC (minimum amount for testing)
	...
	try {
		console.log(`💰 Deposit Amount: ${amount.toString()} raw units (${amount.toNumber() / 1e6} USDC)`);
		console.log(`🏪 Spot Market Index: ${spotMarketIndex}`);

		// Note: This will fail if:
		// 1. User account doesn't exist (run get-drift-user-account.js first)
		// 2. You don't have USDC in your wallet
...
async function settlePnlExample() {
	...
	try {
		...
		if (error.message?.includes('UserHasNoPositionInMarket')) {
			console.log('ℹ️  No positions to settle (this is normal if you have no perp positions)');
		} else {
...
async function runAllExamples() {
	...
	console.log('⚠️  WARNING: This uses REAL FUNDS on mainnet!');
	console.log('   - Deposit/Withdraw: 0.01 USDC ($0.01)');
	console.log('   - Settle operations: No risk if no open positions\n');
